...
def iter_corpus_text(data_frame, as_list=True):
    for idx, row in data_frame.iterrows():
        yield idx, prepair_text(row['text'], 'dutch', 'dutch', as_list=as_list)


def iter_corpus_values(data_frame):
    for idx, row in data_frame.iterrows():
        yield idx, row['target']
...
class FieldClassifier:
    ...
    def __init__(self, data_frame, field, boolean=True, tf=False, tfidf=False):
        ...
        if boolean:
            ...
            input_vectors = tf_vectorizer.fit_transform([text for _, text in iter_corpus_text(data_frame, as_list=False)])
            self.input_vectors = to_boolean_vectorizer(input_vectors)
        elif tf:
            tf_vectorizer = TfidfVectorizer(max_df=0.95, min_df=2, analyzer='word', ngram_range=(1, 1),
                                            max_features=5000, use_idf=False)
            self.input_vectors = tf_vectorizer.fit_transform([text for _, text in iter_corpus_text(data_frame, as_list=False)])
        else:
            tfidf_vectorizer = TfidfVectorizer(max_df=0.95, min_df=2, analyzer='word', ngram_range=(1, 1),
                                               max_features=5000)
            self.input_vectors = tfidf_vectorizer.fit_transform([text for _, text in iter_corpus_text(data_frame, as_list=False)])

        targets = [target for _, target in iter_corpus_values(data_frame)]
